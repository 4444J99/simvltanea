...
(() => {
  ...
  function record(type, node, detail = null) {
    if (armed) events.push({type, wall: performance.now(), token: token(node), // allow-secret
  ...
  function row(video, includePixels) {
    ...
    return {id: video.dataset.loopId, token: token(video), boxToken: token(box), // allow-secret
